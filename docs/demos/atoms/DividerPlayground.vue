...
// Sesuaikan path ini dengan lokasi BaseDivider kamu

// --- STATE ---
const textValue = ref('ATAU');
const alignValue = ref('center'); // start, center, end
const typeValue = ref('solid'); // solid, dashed, dotted
const isVertical = ref(false);
const copied = ref(false);

const alignOptions = ['start', 'center', 'end'];
const typeOptions = ['solid', 'dashed', 'dotted'];

// --- COMPUTED CODE SNIPPET ---
const codeSnippet = computed(() => {
  let props = [];

  if (isVertical.value) props.push('vertical');
  if (!isVertical.value && textValue.value && alignValue.value !== 'center')
    props.push(`align="${alignValue.value}"`);
  if (typeValue.value !== 'solid') props.push(`type="${typeValue.value}"`);

  const propsString = props.length > 0 ? ' ' + props.join(' ') : '';

  if (!isVertical.value && textValue.value) {
    return `<BaseDivider${propsString}>
  ${textValue.value}
</BaseDivider>`;
  } else {
    return `<BaseDivider${propsString} />`;
...
<template>
  ...
  >
    ...
    <div
      class="relative min-h-[200px] flex items-center justify-center p-12 bg-slate-50/50 border-b border-gray-100 transition-all duration-300"
    >
      <div
        v-if="!isVertical"
        class="w-full max-w-md flex flex-col gap-4 text-center text-sm text-gray-500"
      >
        <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
          Konten Atas
        </div>
        <BaseDivider :align="alignValue" :type="typeValue">
          {{ textValue }}
        </BaseDivider>
        <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
          Konten Bawah
      ...
      <div v-else class="flex items-center h-24 text-sm text-gray-500">
        <div
          class="px-6 py-3 bg-white border border-gray-200 rounded-lg shadow-sm"
        >
          Kiri
        </div>
        <BaseDivider vertical :type="typeValue" />
        <div
          class="px-6 py-3 bg-white border border-gray-200 rounded-lg shadow-sm"
        >
          Kanan
    ...
    <div class="p-8 bg-white grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
      <div class="space-y-6">
        ...
        <div :class="{ 'opacity-50 pointer-events-none': isVertical }">
          <label
            class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3"
            >Teks di Tengah Garis</label
          >
          <input
            v-model="textValue"
            type="text"
            ...
            placeholder="Kosongkan jika hanya ingin garis..."
          />
        </div>
      </div>
      ...
      <div class="space-y-6">
        <div
          :class="{
            'opacity-50 pointer-events-none': isVertical || !textValue,
          }"
        >
          <label
            class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3"
            >Posisi Teks (Align)</label
          >
          <div
          ...
          >
            <button
              ...
              :key="align"
              @click="alignValue = align"
              :disabled="isVertical || !textValue"
