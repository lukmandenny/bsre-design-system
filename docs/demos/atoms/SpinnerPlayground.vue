...
// Sesuaikan path ini dengan lokasi BaseSpinner kamu

// --- STATE ---
const sizeValue = ref('xl'); // sm, md, lg, xl
const colorTheme = ref('primary'); // primary, white, success, danger, warning, info, gray
...
const codeSnippet = computed(() => {
  ...
  return `<BaseSpinner${propsString} />`;
});
...
<template>
  ...
  >
    ...
    <div
      :class="[
        'relative min-h-[200px] flex flex-col items-center justify-center p-12 border-b border-gray-100 transition-colors duration-500',
        colorTheme === 'white' ? 'bg-gray-800' : 'bg-slate-50/50',
      ]"
    >
      <div class="transition-all duration-300">
        <BaseSpinner :size="sizeValue" :color="colorTheme" />
      </div>
    </div>
