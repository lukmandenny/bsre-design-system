...
// --- STATE ---
const label = ref('Saya setuju dengan Syarat & Ketentuan');
const hint = ref('Baca dokumen secara seksama sebelum menyetujui.');
const disabled = ref(false);
const bindMode = ref('boolean'); // 'boolean' | 'array'
const customValue = ref('setuju_syarat');
...
// Reset array/boolean state if mode changes
watch(bindMode, () => {
...
const actualModel = computed({
  get: () =>
    bindMode.value === 'boolean' ? booleanValue.value : arrayValue.value,
  set: (val) => {
    if (bindMode.value === 'boolean') {
...
const codeSnippet = computed(() => {
  ...
  if (bindMode.value === 'array') {
    props.push(`v-model="selectedOptions"`);
    props.push(`value="${customValue.value}"`);
  } else {
    props.push(`v-model="isChecked"`);
  }
  ...
  const propsString =
    props.length > 0 ? '\n  ' + props.join('\n  ') + '\n' : ' ';
  return `<BaseCheckbox${propsString}/>`;
});
...
<template>
  ...
  >
    ...
    >
      <div
        class="w-full max-w-sm transition-all duration-300 bg-white p-6 rounded-xl border border-gray-200 shadow-sm"
      >
        <BaseCheckbox
          v-model="actualModel"
          :value="customValue"
      ...
      <div
        class="mt-6 px-4 py-2 bg-white rounded-lg border border-gray-200 shadow-sm text-[13px] font-mono text-gray-600 flex items-center gap-2"
      >
        <span class="text-gray-400 select-none">v-model :</span>
        <span
          v-if="bindMode === 'boolean'"
          :class="
            actualModel
              ? 'text-[#10B981] font-bold'
              : 'text-[#D91744] font-bold'
          "
        >
          {{ actualModel }}
        </span>
        <span v-else class="text-[#2FAAE1] font-bold">
          {{ JSON.stringify(actualModel) }}
        </span>
      </div>
    </div>
    ...
    <div class="p-8 bg-white grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
      <div class="space-y-6">
        <div>
          ...
          <input
            ...
            placeholder="Masukkan label checkbox..."
          />
        </div>
        <div>
          <label
          ...
          <input
            ...
            type="text"
            class="w-full rounded-xl bg-gray-50 text-[15px] focus:outline-none focus:ring-1 focus:ring-[#2FAAE1] py-3 px-4 text-gray-700 border border-gray-100 hover:bg-gray-100/50 transition-colors"
            placeholder="Masukkan teks bantuan tambahan..."
          />
        </div>
      </div>
      ...
      <div class="space-y-6">
        <div>
          ...
          >
            <button
              @click="bindMode = 'boolean'"
              class="px-5 py-2 text-[13px] rounded-full transition-all duration-200 font-medium"
              :class="
                bindMode === 'boolean'
        ...
        <div v-if="bindMode === 'array'" class="animate-fade-in">
          <label
            class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3"
            >Bind Value</label
          >
          <input
            ...
            type="text"
            class="w-full rounded-xl bg-gray-50 text-[15px] focus:outline-none focus:ring-1 focus:ring-[#2FAAE1] py-3 px-4 text-gray-700 border border-gray-100 hover:bg-gray-100/50 transition-colors"
            placeholder="Nilai yang dikirim ke array..."
          />
        </div>
        ...
        <div>
          ...
          >
            ...
            <span class="text-[14px] text-gray-700 font-medium">Disabled</span>
          </label>
        </div>
      </div>
    </div>
