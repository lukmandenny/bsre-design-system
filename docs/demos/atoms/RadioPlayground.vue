...
// Pastikan path ini sesuai dengan lokasi komponen BaseRadio kamu

// --- STATE ---
const selectedValue = ref('opsi_1');

// Opsi 1
const label1 = ref('Pilihan Pertama');
const value1 = ref('opsi_1');
const hint1 = ref('Deskripsi tambahan untuk opsi pertama.');

// Opsi 2
const label2 = ref('Pilihan Kedua');
const value2 = ref('opsi_2');
...
const codeSnippet = computed(() => {
  const buildProps = (val, lbl, hnt) => {
    let props = [`v-model="selectedValue"`, `value="${val}"`];
    if (lbl) props.push(`label="${lbl}"`);
    if (hnt) props.push(`hint="${hnt}"`);
  ...
  <div class="flex flex-col gap-4">
    <BaseRadio ${buildProps(value1.value, label1.value, hint1.value)} />
    <BaseRadio ${buildProps(value2.value, label2.value, '')} />
  </div>
</template>
  ...
  >
    ...
    >
      ...
      >
        ...
        <span class="text-[#2FAAE1] font-bold"> "{{ selectedValue }}" </span>
      </div>
    </div>
